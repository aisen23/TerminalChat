...
namespace tc
{
	namespace log
	{
		...
		void init(std::string_view instanceId)
		{
				...
				while (std::getline(file, line))
				{
					if (line.starts_with("LogLevel="))
					{
						if (line.find("Debug") != std::string::npos)
							g_logLevel = Level::Debug;
						else if (line.find("Info") != std::string::npos)
							g_logLevel = Level::Info;
						else if (line.find("Warn") != std::string::npos)
							g_logLevel = Level::Warn;
						else if (line.find("Error") != std::string::npos)
							g_logLevel = Level::Error;
						else if (line.find("Fatal") != std::string::npos)
							g_logLevel = Level::Fatal;
					}
					else if (line.starts_with("MaxFileSize="))
					{
						try
						{
							...
							if (val > 0)
								g_maxFileSizeBytes = val * 1024 * 1024;
						}
						catch (...)
						{
						}
			...
			const std::string logFile = g_binName + (instanceId.empty() ? ".log" : std::format(".{}.log", instanceId));
